...
SEARCH_URL = "https://cn.bzmgcn.com/search?q="

# Selenium浏览器配置


def get_driver():
    # 自动安装匹配 Chromium 版本的 chromedriver
    print("[DEBUG] 开始下载/检查 chromedriver...")
    chromedriver_autoinstaller.install()
    print("[DEBUG] chromedriver 准备完成")
    chrome_options = Options()
    chrome_options.binary_location = r"C:\ScoopApps\shims\chromium.exe"  # 使用 Chromium
    print("[DEBUG] 设置代理...")
...
class BaozimhGUI:
    def __init__(self, master):
        ...
        self.main_title = Label(
            master, text="包子漫画爬虫", font=title_font, bg="#f8f8f8"
        )
        ...
        self.search_btn = Button(
            input_frame, text="搜索", width=10, command=self.start_search
        )
        ...
        self.result_label = Label(
            result_frame, text="搜索结果:", font=result_label_font, bg="#f8f8f8"
        )
        ...
        self.info_label = Label(
            info_frame, text="漫画信息:", font=result_label_font, bg="#f8f8f8"
        )
        ...
        self.copy_label = Label(
            copy_frame, text="可复制字段:", font=result_label_font, bg="#f8f8f8"
        )
        self.copy_label.pack(anchor="w", pady=(0, 2))
        self.copy_text = Text(
            copy_frame, width=80, height=5, wrap="none", font=default_font
        )
        self.copy_text.pack(fill="x", pady=(0, 2))
        self.copy_text.config(state="disabled")
        # 日志区
        log_frame = Frame(master, bg="#f8f8f8")
        log_frame.pack(fill="both", padx=20, pady=(10, 5), expand=True)
        self.log_label = Label(
            log_frame, text="运行日志:", font=result_label_font, bg="#f8f8f8"
        )
    ...
    def start_search(self):
        ...
        self.search_thread = threading.Thread(
            target=self.search, args=(keyword,), daemon=True
        )
            ...
            for card in cards:
                ...
                try:
                    title = card.find_element(
                        By.CSS_SELECTOR, ".comics-card__title h3"
                    ).text.strip()
                    author = card.find_element(
                        By.CSS_SELECTOR, "small.tags.text-truncate"
                    ).text.strip()
                    cover_url = card.find_element(
                    ...
                    if detail_url and "call_page('" in detail_url:
                        ...
                        detail_url = (
                            "https://appcn1-vdkr.baozimh.com/baozimhapp/" + detail_url
                        )
                    else:
    ...
    def on_select(self, event):
        ...
        if not detail_url:
            ...
            return
        threading.Thread(
            target=self.parse_detail,
            args=(title, author, cover_url, detail_url),
            daemon=True,
        ).start()
    ...
    def parse_detail(self, title, author, cover_url, detail_url):
        ...
        try:
            ...
            WebDriverWait(self.driver, 15).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "h1.comics-detail__title")
                )
            )
        except Exception:
        ...
        # 解析标题
        try:
            real_title = self.driver.find_element(
                By.CSS_SELECTOR, "h1.comics-detail__title"
            ).text.strip()
        except Exception:
            real_title = title
        # 解析作者
        try:
            real_author = self.driver.find_element(
                By.CSS_SELECTOR, "h2.comics-detail__author"
            ).text.strip()
        except Exception:
            real_author = author
        # 解析简介
        try:
            desc = self.driver.find_element(
                By.CSS_SELECTOR, "p.comics-detail__desc"
            ).text.strip()
        except Exception:
            desc = ""
        # 若有"查看全部章节"按钮则点击
        ...
        try:
            ...
            try:
                ...
                chapters1 = chapter_items.find_elements(
                    By.CSS_SELECTOR, ".comics-chapters__item"
                )
            ...
            try:
                ...
                WebDriverWait(self.driver, 5).until(
                    lambda d: (
                        chapters_other.is_displayed()
                        or chapters_other.get_attribute("style") == ""
                    )
                )
                chapters2 = chapters_other.find_elements(
                    By.CSS_SELECTOR, ".comics-chapters__item"
                )
        ...
        BASE_URL = (
            f"https://app.baozimh.com/baozimhapp/comic/chapter/{title_py}-{author_py}/"
        )
        COVER_URL = cover_img
        CHAPTER_START = 0
